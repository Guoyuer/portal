"""EmpowerSource — snapshot-level 401k broker.
...
class EmpowerSourceConfig:
    downloads_dir: Path
    cusip_map: dict[str, str] = field(default_factory=lambda: dict(_DEFAULT_CUSIP_MAP))
...
class EmpowerSource:
    ...
    @classmethod
    def from_raw_config(cls, raw: dict[str, object], db_path: Path) -> EmpowerSource:
        """Build an EmpowerSource from the raw config dict.

        Reads ``empower_downloads`` (QFX directory). If the key is missing the
        source falls back to a non-existent path, so :meth:`ingest` becomes a
        silent no-op. An optional ``empower_cusip_map`` key overrides the
        built-in :data:`_DEFAULT_CUSIP_MAP`.
        """
        raw_path = raw.get("empower_downloads")
        if isinstance(raw_path, (str, Path)):
            downloads_dir = Path(raw_path)
        else:
            downloads_dir = Path("__missing_empower_downloads__")
        cusip_raw = raw.get("empower_cusip_map")
        cusip_map = dict(cusip_raw) if isinstance(cusip_raw, dict) else dict(_DEFAULT_CUSIP_MAP)
        return cls(EmpowerSourceConfig(downloads_dir=downloads_dir, cusip_map=cusip_map), db_path)

    # ── Ingest ───────────────────────────────────────────────────────────

    def ingest(self) -> None:
        """Scan ``downloads_dir`` for ``Bloomberg.Download*.qfx`` and ingest each file.

        Populates ``empower_snapshots`` + ``empower_funds`` (idempotent per
        snapshot date: INSERT OR IGNORE the snapshot, DELETE + INSERT the
        funds) and ``empower_contributions`` (deduped by
        ``(date, amount, ticker, cusip)``).

        Silent no-op when the downloads directory doesn't exist — mirrors
        :class:`RobinhoodSource`'s missing-CSV behaviour.
        """
        if not self._config.downloads_dir.exists():
            return
        qfx_paths = sorted(self._config.downloads_dir.glob("Bloomberg.Download*.qfx"))
        if not qfx_paths:
            return

        for qfx_path in qfx_paths:
            self._ingest_one_qfx(qfx_path)

        # Contributions: load + dedup + persist in one pass.
        self._ingest_contributions_from_qfx(qfx_paths)

    def _ingest_one_qfx(self, qfx_path: Path) -> None:
        """Write a single QFX's snapshot + fund rows. Idempotent by snapshot_date."""
        snap = _parse_qfx(qfx_path, self._config.cusip_map)
        if not snap.funds:
            return
    ...
    def _ingest_contributions_from_qfx(self, qfx_paths: list[Path]) -> None:
        """Load BUYMF rows from every QFX, dedup across files, and persist."""
        all_contribs: list[Contribution] = []
        for qfx_path in qfx_paths:
            all_contribs.extend(_parse_qfx_contributions(qfx_path, self._config.cusip_map))
        if not all_contribs:
            return
        ...
        for c in sorted(all_contribs, key=lambda x: x.date):
            ...
            seen.add(key)
            unique.append(c)
        self.ingest_contributions(unique)

    def ingest_contributions(self, contribs: list[Contribution]) -> None:
        """Upsert contributions into ``empower_contributions``.

        Public so the build script can also route Qianji-derived fallback
        contributions (for periods without a QFX file) through the same
        table that :meth:`positions_at` reads. Deduplicates on the primary
        key ``(date, amount, ticker, cusip)``.
        """
        if not contribs:
            return
        conn = get_connection(self._db_path)
    ...
    def positions_at(self, as_of: date, prices: PriceContext) -> list[PositionRow]:
        ...
        1. Find latest snapshot with ``snapshot_date <= as_of``. If none,
           return ``[]`` (before-first-snapshot is a real case during
           historical replay).
        2. For each fund in that snapshot, scale ``mktval`` by
           ``proxy_price(as_of) / proxy_price(snapshot_date)`` — falling back
           to the raw ``mktval`` if either proxy price is unavailable or the
           fund has no proxy mapping.
        ...
        # Aggregate by config ticker (a snapshot may have multiple funds with
        # the same config ticker — e.g., two S&P 500 variants).
...
if EmpowerSource not in _REGISTRY:
    _REGISTRY.append(EmpowerSource)


_: type[InvestmentSource] = EmpowerSource  # structural-subtype sanity check
