...
// parse mismatches on real payloads.
...
test.describe("Real-worker smoke", () => {
  ...
  test("/timeline returns non-empty rows + expected top-level keys", async ({ request }) => {
    // Direct API assertion: the Worker streams endpoint artifacts matching the
    // keys the frontend Zod schema expects. No mock indirection.
  ...
  test("net-worth number rendered from real data (has-recent-data)", async ({ page }) => {
    // Exercises the full path: wrangler → R2 artifact → Next static bundle's
    // Zod parse → component render. If any layer drifts, the tile either fails
    // to render or shows N/A, both of which this catches.
    await page.goto("/finance");
