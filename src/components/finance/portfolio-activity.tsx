    ...
    <>
      ...
      {reconciliation && (
        <div className="flex flex-wrap items-center gap-2 mb-4 py-2 px-3 rounded-md bg-muted text-sm">
          ...
          <span
            className={`font-semibold ${reconciliation.totalChange >= 0 ? "text-green-600" : "text-red-500"}`}
          >
        ...
        <TableBody>
          {rows.map((row) => (
            <TableRow key={row.label} className="even:bg-muted/50">
              ...
              <TableCell
                className={`text-right ${row.indent ? "text-muted-foreground" : row.amount >= 0 ? "text-green-600" : "text-red-500"}`}
              >
